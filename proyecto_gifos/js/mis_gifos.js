...
function mostrarGifos(array) {
  //es como la funcion favoritos() pero modificada en algunas partes
...
verMasGifos.addEventListener("click", () => {
  if (arraySeccionGifos.length > 12) {
    ...
    for (let r = contGif; r < arraySeccionGifos.length; r++) {
      crearTarjeta(
        arraySeccionGifos[r].images.fixed_height_downsampled.url,
        arraySeccionGifos[r].title,
        arraySeccionGifos[r].username,
      ...
      linkDescargaGifos[r].setAttribute(
        "href",
        arraySeccionGifos[r].images.original.url
      );
      linkDescargaGifos[r].download = "descarga.gif";
  ...
  for (let l = 0; l < arraySeccionGifos.length; l++) {
    if (imgMaximizar[l].className === "maximizar gifos activado") {
      crearTarjetaMaximizada(
        arraySeccionGifos[l].images.fixed_height.url,
        arraySeccionGifos[l].title,
        arraySeccionGifos[l].username,
        arraySeccionGifos[l].images.original.url,
        "gifs-max-gifos",
        "eliminar-max",
        "itemBusqueda"
      );
