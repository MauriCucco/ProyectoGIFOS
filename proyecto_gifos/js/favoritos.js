...
let galeriaImagenesFavoritos = document.querySelector(
...
function favoritos(array) {
  if (!array || array.length === 0) {
    ...
    for (let r = 0; r < array.length; r++) {
      crearTarjeta(
        array[r].images.fixed_height_downsampled.url,
        array[r].title,
        array[r].username,
        ".galeria-imagenes-favoritos",
        "maximizar seccion",
      ...
      iconSeccionFavoritos[r + cont].setAttribute(
        "src",
...
verMasFavoritos.addEventListener("click", () => {
  if (arraySeccionFavoritos.length > 12) {
    ...
    for (let r = contFav; r < arraySeccionFavoritos.length; r++) {
      crearTarjeta(
        arraySeccionFavoritos[r].images.fixed_height_downsampled.url,
        arraySeccionFavoritos[r].title,
        arraySeccionFavoritos[r].username,
        ".galeria-imagenes-favoritos",
        "maximizar seccion",
        "favorito",
        "descarga",
      ...
      linkDescarga[r + cont].setAttribute(
        "href",
        arraySeccionFavoritos[r].images.original.url
      );
      linkDescarga[r + cont].download = "descarga.gif";
...
function borrarFavoritoSeccion() {
  let iconSeccionFavoritos = document.getElementsByClassName(
    "favorito seccion"
  );
  let indice;
  ...
  if (indexFavoritoTrending) {
    iconFavoritoTrending[indexFavoritoTrending].setAttribute(
      "src",
    ...
    posicionFavTrendMax.splice(
      posicionFavTrendMax.indexOf(indexFavoritoTrending),
      1
    ); //borro la posicion de FavTrendMax
...
function maximizarFavorito() {
  ...
  for (let l = 0; l < arraySeccionFavoritos.length; l++) {
    if (
      imgMaximizar[l].className == "maximizar seccion activado" ||
      imagenSeccion[l].className == "gifs seccion activado"
    ) {
      crearTarjetaMaximizada(
        arraySeccionFavoritos[l].images.fixed_height.url,
        arraySeccionFavoritos[l].title,
        arraySeccionFavoritos[l].username,
        arraySeccionFavoritos[l].images.original.url,
        "gifs-max-seccion",
...
function mantenerActivadoSeccion() {
  let iconMaxSeccion = document.querySelector(
    "img#itemBusqueda.favorito.seccion-max"
  );
...
function tildarMaxFavorito() {
  let iconFavMax = document.querySelector(
    "img#itemBusqueda.favorito.seccion-max"
  );
  let titulosGifoSeccion = document.getElementsByClassName(
    "titulo-gifo seccion"
  );
  let iconSeccionFavoritos = document.getElementsByClassName(
    "favorito seccion"
